...
import type { LpItem, Comment } from '../types/lp';
...
export default function LpDetailPage() {
    ...
    const commentsKey = ['lpComments', lpid, commentOrder];
    ...
    const createCommentMutation = useMutation({
        mutationFn: async (content: string) => {
            const response = await apiClient.post(`/lps/${lpid}/comments`, { content });
    ...
    const updateCommentMutation = useMutation({
        mutationFn: async ({ commentId, content }: { commentId: number; content: string }) => {
            const response = await apiClient.patch(`/lps/${lpid}/comments/${commentId}`, { content });
            return response.data?.data || response.data;
        },
        onSuccess: () => {
    ...
    const likeMutation = useMutation({
        ...
        onMutate: async (liked: boolean) => {
            ...
            const previous = queryClient.getQueryData<LpItem>(['lp', lpid]);
            queryClient.setQueryData<LpItem>(['lp', lpid], (old) => {
                if (!old) return old;
                const likes = liked
                    ? old.likes.filter((l) => l.userId !== currentUserId)
                    : [...old.likes, { id: Date.now(), userId: currentUserId, lpId: old.id }];
                return { ...old, likes };
            });
            return { previous };
        },
    ...
    const isLpOwner = currentUserId != null && (lp.authorId === currentUserId || lp.author?.id === currentUserId);
        ...
        <div className="min-h-[calc(100vh-72px)] bg-black px-4 py-8 flex justify-center">
            <div className="w-full max-w-3xl bg-[#232428] rounded-[2rem] p-8 md:p-12 shadow-2xl">
                ...
                <div className="flex flex-wrap items-center justify-center gap-3 mb-12">
                    {lp.tags?.length > 0 ? lp.tags.map((tag) => (
                        <span key={tag.id} className="px-4 py-1.5 bg-zinc-800 text-zinc-400 rounded-full text-sm font-semibold">
                            #{tag.name}
                        </span>
                    )) : (
                ...
                <div className="flex justify-center border-t border-zinc-800 pt-8">
                    <button
                        ...
                        className={`flex items-center gap-3 text-3xl font-bold transition-colors group ${isLiked ? 'text-rose-500' : 'text-zinc-400 hover:text-rose-500'}`}
                        aria-label="좋아요"
                    >
                        <svg
                            className={`w-10 h-10 group-hover:scale-110 transition-transform ${isLiked ? 'text-rose-500' : 'text-zinc-500'}`}
                        ...
                        {isCommentsPending ? (
                            ...
                            <>
                                {allComments.map((comment, idx) => {
                                    const isOwner = currentUserId != null && (comment.authorId === currentUserId || comment.author?.id === currentUserId);
                                                        ...
                                                        {isOwner && !isEditing && (
                                                            <div className="relative">
                                                                <button
                                                                    ...
                                                                    className="text-zinc-500 hover:text-white px-1 transition-colors"
                                                                    aria-label="댓글 메뉴"
                                                                >
                                                                    ⋯
                                                                </button>
                                                                {openMenuId === comment.id && (
                                                                    <div className="absolute right-0 mt-1 w-24 bg-zinc-800 border border-zinc-700 rounded-lg shadow-xl z-10 overflow-hidden">
                                                                        <button
                                                                            onClick={() => {
                                                                        ...
                                                                        <button
                                                                            ...
                                                                            className="block w-full text-left px-4 py-2 text-sm text-rose-400 hover:bg-zinc-700 transition-colors"
                                                                        >
                                                                            삭제
                                                                        </button>
                                                                    </div>
            ...
            {editModalOpen && (
                <LpFormModal lp={lp} onClose={() => setEditModalOpen(false)} />
            )}
        </div>
    );
}
